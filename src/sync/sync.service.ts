...
export class SyncService {
  ...
  async getAgroRules(since?: string): Promise<{ schemaVersion: number; updatedAt: string; rules: AgroRule[] }> {
    const qb = this.agroRuleRepo.createQueryBuilder('r').where('r.actif = true');
  ...
  async createExpertRequest(data: any): Promise<{ id: string; status: string; receivedAt: string }> {
    const request = await this.expertRequestRepo.save(
      this.expertRequestRepo.create(data),
    );
    return { id: request.id, status: 'received', receivedAt: new Date().toISOString() };
  }

  async pushSync(actions: any[]): Promise<{ processed: number; errors: any[]; conflicts: any[]; serverTimestamp: string }> {
    const errors: any[] = [];
    let processed = 0;
    ...
    return { processed, errors, conflicts: [], serverTimestamp: new Date().toISOString() };
  }

  async pullSync(since: string, resources: string): Promise<{ parcels: any[]; agro_rules: any[]; serverTimestamp: string }> {
    const resourceList = resources.split(',');
    const result: any = { parcels: [], agro_rules: [], serverTimestamp: new Date().toISOString() };

    if (resourceList.includes('parcelles') || resourceList.includes('parcels')) {
      const qb = this.parcelleRepo.createQueryBuilder('p').where('p.deleted = false');
      if (since) qb.andWhere('p.updatedAt >= :since', { since: new Date(since) });
      const parcelles = await qb.getMany();
      result.parcels = parcelles.map(this.toFlutterParcel);
    }

    if (resourceList.includes('agro_rules')) {
      result.agro_rules = await this.agroRuleRepo.find({ where: { actif: true } });
    }
  ...
  private toFlutterParcel(p: Parcelle): any {
    ...
    return {
      ...
      boundary: boundary?.coordinates?.[0]?.slice(0, -1).map(([lng, lat]: [number, number]) => [lat, lng]) || [],
      variety: p.variete,
