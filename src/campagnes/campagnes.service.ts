...
export class CampagnesService {
  ...
  async findAll(query?: {
    ...
    if (query?.organisationId)
      qb.andWhere("c.organisationId = :org", { org: query.organisationId });
  ...
  async cloture(
    ...
    campagne.dateFin = new Date(
      data.dateFin || new Date().toISOString().split("T")[0],
    );
    if (data.rendementFinal) campagne.rendementFinal = data.rendementFinal;
    if (data.observationsCloture)
      campagne.observationsCloture = data.observationsCloture;
    return this.campagneRepo.save(campagne);
  }
