...
export class VisitesService {
  ...
  async findAll(query?: {
    ...
    if (query?.organisationId)
      qb.andWhere("v.organisationId = :org", { org: query.organisationId });
    if (query?.technicianId)
      qb.andWhere("v.technicienId = :tech", { tech: query.technicianId });
    if (query?.statut)
      qb.andWhere("v.statut = :statut", { statut: query.statut });
  ...
  async findStats(organisationId?: string): Promise<VisiteStats> {
    ...
    if (organisationId)
      base.where("v.organisationId = :org", { org: organisationId });
